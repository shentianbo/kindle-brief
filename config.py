...
ARTICLE_PROMPT = """请对以下「{chapter_title}」章节的文章进行简报整理。

{"心理学章节特殊要求（is_psych=是）：每篇文章需要五个层次：背景理论解释（从零开始，不假设读者有专业背景）、研究发现（具体数据和方法）、核心概念深度展开（真正的分析）、延伸维度（一个额外视角）、三个递进层次的延伸思考问题。" if "{is_psych}" == "是" else "普通章节格式：每条文章严格按照固定格式输出。"}

{"普通章节" if "{is_psych}" == "否" else "心理学章节"}每条文章格式：
...
{"心理学章节在以上基础上增加：" if "{is_psych}" == "是" else ""}
{"背景理论：（解释核心心理学概念，从历史和学术来源入手，200字左右）" if "{is_psych}" == "是" else ""}
{"核心展开：（真正的分析，不只是结论，解释机制和原因，150-200字）" if "{is_psych}" == "是" else ""}
{"延伸维度：（一个额外的视角，可以是神经科学、文化差异、与其他理论的联结等，100字）" if "{is_psych}" == "是" else ""}
{"延伸思考：（三个递进问题，表层→中层→深层，引导读者自我代入）" if "{is_psych}" == "是" else ""}
